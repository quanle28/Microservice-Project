...
import com.aql.userservice.service.UserService;
...
//		when(userRepository.saveAll((Iterable<S>) any(User.class))).thenReturn(user);
...
//	}
	...
	void login() {
		...
		when(passwordEncoder.matches(toString(), toString())).thenReturn(true);
	...
	void loginWithPasswordNotMatch() {
		when(userRepository.findByUsername(user.getUsername())).thenReturn(user);
		when(passwordEncoder.matches(toString(), toString())).thenReturn(false);
		Assertions.assertNotNull(userService.login(user.getUsername(), user.getPassword()));
	}
